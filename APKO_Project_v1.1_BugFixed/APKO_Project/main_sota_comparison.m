...
%% BUG-1 FIX: FE budget tracking (same as main APKO.m)
MaxFES   = Popsize * Maxiteration;
FE_count = 0;

Fitness = zeros(1,Popsize);
for i=1:Popsize
    Fitness(i)=Fobj(X(i,:)); FE_count=FE_count+1;
end
...
%% BUG-3 FIX: deterministic angle baseline for ablation variant
if flags.fix_angle
    Fixed_Crest = 2*pi*rand;   % Single draw at start — ablation baseline
end
...
while t <= Maxiteration && FE_count < MaxFES
    ...
    %% BUG-3 FIX: deterministic two-frequency schedule when not ablated
    if flags.fix_angle
    ...
    for i=1:Popsize
        if FE_count>=MaxFES; break; end
        X_1(i,:)=Clamp(X_1(i,:),LB,UB); fitnessn(i)=Fobj(X_1(i,:)); FE_count=FE_count+1;
    ...
    for i=1:Popsize
        ...
        if rand>(1-PE)
            ...
            X_1(i,:)=Clamp(X_1(i,:),LB,UB); fitnessn(i)=Fobj(X_1(i,:)); FE_count=FE_count+1;
            if fitnessn(i)>=Fitness(i) && ~flags.no_cauchy && FE_count<MaxFES
                cp=X(i,:)+cauchy_scale*tan(pi*(rand(1,Dim)-0.5));
                cp=Clamp(cp,LB,UB); cf=Fobj(cp); FE_count=FE_count+1;
    ...
    if ~flags.no_entropy
        ...
        if t>entropy_window && (mean(entropy_history)/log(Popsize))<H_entr
            num_r=max(1,round(k_restart*Popsize));
            if FE_count+num_r <= MaxFES
                [~,wi]=sort(Fitness,'descend');
                for r=1:num_r
                    if FE_count>=MaxFES; break; end
                    idx=wi(r); X(idx,:)=Best_position+randn(1,Dim).*abs(UB-LB)*0.1*(1-t/Maxiteration);
                    X(idx,:)=Clamp(X(idx,:),LB,UB); Fitness(idx)=Fobj(X(idx,:)); FE_count=FE_count+1;
